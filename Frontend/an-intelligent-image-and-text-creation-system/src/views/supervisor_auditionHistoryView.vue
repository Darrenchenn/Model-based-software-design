...
const filteredProducts = computed(() => {
  ...
  return products.value.filter((product) => {
    return product.responsible_supervisor_uuid === supervisor_userId
  })
})
...
const toggleReviewDialog = (product) => {
  if (product === selectedProduct.value && showReviewDialog) {
          ...
          <div class="list-item" @click="() => toggleReviewDialog(product)">
            <div class="list-item-content">
              <div class="list-item-left">
                ...
                <p class="p1_a">
                  {{ product.audit_comment ? product.audit_comment : 'No Comment Yet' }}
                </p>
                <p>Audition Status:</p>
              ...
              <div class="list-item-right">
                ...
                <img
                  ...
                  :style="{
                    width: selectedProduct === product ? '80%' : 'auto',
                    margin: selectedProduct === product ? '0 auto' : '0 auto'
                  }"
                  @click.stop
                />
              </div>
          ...
          <div v-if="product === selectedProduct && showReviewDialog" class="review-dialog">
            <div style="display: flex; width: 100%">
              <textarea
                class="ta_a"
                v-model="reviewText"
                :placeholder="product.audit_comment ? product.audit_comment : 'Please comment'"
              ></textarea>
              <div style="display: flex; flex-direction: column; padding-left: 5%">
                <select
                  id="auditStatusSelect"
                  v-model="auditStatus"
                  style="
                    padding-bottom: 5px;
                ...
                <button
                  @click="submitReview"
                  style="background-color: #303237; color: white; border-radius: 3px"
                >
                  CONFIRM
                </button>
                <button
                  @click="cancelAudition(product)"
                  style="background-color: #303237; color: white; border-radius: 3px"
                >
                  Audition Cancel
                </button>
