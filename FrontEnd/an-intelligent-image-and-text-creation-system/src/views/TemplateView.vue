...
onMounted(() => {
  axios.get(`http://127.0.0.1:8000/get_all_templates/?page=${currentPage.value}&page_size=${pageSize.value}`)
    .then((res) => {
...
const addTemplate = () => {
  ...
  axios.post('http://127.0.0.1:8000/create_template/', newTemplateData, {
    headers: {
...
const submitReview = () => {
  if (selectedTemplate) {
    ...
    axios.post(`http://127.0.0.1:8000/update_template_by_uuid/${selectedTemplate.value.uuid}/`, reviewData)
      .then((res) => {
...
const deleteTemplate = () => {
  ...
  if (selectedTemplate && selectedTemplate.value.uuid !== null && selectedTemplate.value.uuid !== undefined) {
    // 使用 HTTP DELETE 请求来删除模板
    console.log(selectedTemplate.value.uuid)
    axios.get(`http://127.0.0.1:8000/delete_template/${selectedTemplate.value.uuid}`)
      .then((res) => {
        if (res.status === 200) {
...
const toggleReviewDialog = (template) => {
  if (template === selectedTemplate && showReviewDialog) {
...
<template>
  <div class="div_a">
    ...
    <h2 class="h1_d">Create the new template</h2>

    <!-- 显示模板列表 -->
    <div class="list-container">
      <div class="add_class">
          <textarea class="ta_b" v-model="newTemplateContent" placeholder="New Template Content"></textarea>
      ...
      <div class="pagination">
        <button @click="changePage(currentPage - 1)" :disabled="currentPage.value === 0">Previous</button>
